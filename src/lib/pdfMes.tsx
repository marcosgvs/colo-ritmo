...
/** ColoMark v3 · paths vetoriais extraídos do SVG do site. Lavender. */
const COLO_MARK_PATHS: Array<{ d: string; fill: [number, number, number] }> = [
  // Rosto + ícone (lavender claro)
  { d: 'M94.98,13.83c.69-1.44,1.95-1.97,2.95-1.78,1.66.48,2.26,1.88,1.88,3.51-1.14,5.17-10.47,38.22-12.64,42.4-.72,1.38-2.22,1.79-3.43,1.43s-2.41-1.55-2.14-3.17c.52-3.6,11.19-37.79,13.38-42.39Z', fill: [162, 153, 203] },
...
function parseSvgPath(
  ...
  const out: Array<{ op: string; c?: number[] }> = [];
  ...
  function tx(v: number) { return ox + v * scale; }
  function ty(v: number) { return oy + v * scale; }
  ...
  while (i < tokens.length) {
    ...
    const next = (n: number) => {
      const arr: number[] = [];
      for (let k = 0; k < n; k++) arr.push(parseFloat(tokens[i++]!));
      return arr;
    };
    switch (cmd) {
      case 'M': {
      ...
      case 'C': {
        ...
        cx = x!;
        cy = y!;
        out.push({ op: 'c', c: [tx(x1!), ty(y1!), tx(x2!), ty(y2!), tx(cx), ty(cy)] });
        break;
      }
      case 'c': {
        const [x1, y1, x2, y2, x, y] = next(6);
        out.push({ op: 'c', c: [tx(cx + x1!), ty(cy + y1!), tx(cx + x2!), ty(cy + y2!), tx(cx + x!), ty(cy + y!)] });
        cx += x!;
        cy += y!;
        break;
      }
      ...
      case 'z':
        ...
        break;
      default:
        // Comando não suportado — pula um número
        next(1);
        break;
    }
  }
