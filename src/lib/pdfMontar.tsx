 ...
 *
 * Layout (A4 landscape):
 *   - Header: logo Colo/Ritmo + eyebrow "SUGESTÃO DE ESCALA" + título
 *     grande do hospital (Fraunces) + subtitle (mês · plantões · nome)
 *   - Saudação: "Prezado(a) Dr(a). [Chefe], ..."
 *   - Coluna esquerda 2/3: calendário do mês com bloquinhos
 *   - Coluna direita 1/3: detalhamento dos plantões em texto
 *   - Total no rodapé direito
 *   - Assinatura em Caveat italic
 *   - Footer: "Documento gerado pelo Colo Ritmo · DD/MM/YYYY"
 *
 * Decisões importantes:
 *   - PDF NÃO inclui bloqueios pessoais · só plantões propostos
 *     do hospital específico. Bloqueios são privados da médica.
 *   - Vetorial puro (jspdf nativo) · texto copiável · ~150kb
 *   - Fontes embedadas (Fraunces, Nunito, Caveat) · TTFs em /public/fonts/
 *   - Logo Colo/Ritmo: SVG inline → PNG via canvas (~5-10kb)
 *   - Português padrão (não minúsculo) · saudação ao chefe · saída externa
 *     formal seguindo `feedback_voz.md`.
 */
...
const CORES_HEX: Record<string, { ink: string; surface: string }> = {
...
const COR_INK = '#2C1F00';
const COR_INK_2 = '#5C5040';
const COR_INK_3 = '#8C7E6A';
const COR_LINHA = '#E8DDC9';
const COR_LAVENDER = '#5A4E8C';

// Logo SVG inline · cópia simplificada do ColoMark v3
...
export async function baixarPDFMontar(dados: DadosPDF): Promise<void> {
  ...
  // --- HEADER ---
  pdf.addImage(logoPng, 'PNG', margemX, y, 38, 8.7);
  y += 12;

  pdf.setFont('Nunito', 'bold');
  pdf.setFontSize(8);
  pdf.setTextColor(COR_INK_3);
  pdf.text('SUGESTÃO DE ESCALA', margemX, y, { charSpace: 1.2 });

  y += 5;
  pdf.setFont('Fraunces', 'bold');
  pdf.setFontSize(28);
  pdf.setTextColor(COR_LAVENDER);
  pdf.text(dados.hospital.nome, margemX, y + 8);
  ...
  const subtitle = `${capitalize(mesNome)} de ${dados.ano} · ${dados.plantoes.length} plantões · ${dados.preferencias.nome ?? 'Médica'}`;
  ...
  y += 5;
  const saudacao = `Apresento abaixo a proposta de plantões para ${mesNome} de ${dados.ano} no ${dados.hospital.nome}, conforme minha disponibilidade. Fico à disposição para os ajustes que forem necessários.`;
  ...
  const calX = margemX;
  const detX = margemX + calLargura + 6;
  ...
  pdf.setFontSize(20);
  pdf.setTextColor(COR_INK);
  pdf.text(dados.preferencias.nome ?? 'Médica', margemX, assinaturaY + 8);
  ...
  // Save
  const arquivoNome = `colo-ritmo_${(dados.hospital.abrev ?? 'hospital').toLowerCase()}_${dados.ano}-${String(dados.mes).padStart(2, '0')}.pdf`;
...
function desenharCalendario(
  ...
  for (let i = 0; i < 7; i++) {
    pdf.text(dows[i] ?? '', x + i * colW + colW / 2, y + 4, { align: 'center' });
  }

  // Borda externa
  pdf.setDrawColor(COR_LINHA);
  pdf.setLineWidth(0.3);
  pdf.roundedRect(x, y + headH, largura, linhaH * semanas, 2, 2);
  ...
  const corHosp = CORES_HEX[dados.hospital.cor] ?? CORES_HEX.lavender!;
  ...
  for (let i = 0; i < dias.length; i++) {
    ...
    pdf.text(String(dia), cx + 1.6, cy + 4.5);
    ...
    for (const p of lista.slice(0, 2)) {
      ...
      const horaIni = `${String(Math.floor(p.horaInicio)).padStart(2, '0')}:${String(Math.round((p.horaInicio % 1) * 60)).padStart(2, '0')}`;
      const horaFim = `${String(Math.floor(fim)).padStart(2, '0')}:${String(Math.round((fim % 1) * 60)).padStart(2, '0')}`;

      // Background pill
      pdf.setFillColor(corHosp.surface);
      pdf.setDrawColor(corHosp.ink);
      pdf.setLineWidth(0.3);
      pdf.roundedRect(cx + 1.2, by, colW - 2.4, 7, 1, 1, 'FD');
      // Tira lateral colorida
      pdf.setFillColor(corHosp.ink);
      pdf.rect(cx + 1.2, by, 0.8, 7, 'F');

      // Texto
      pdf.setFont('Nunito', 'bold');
      pdf.setFontSize(6.5);
      pdf.setTextColor(corHosp.ink);
      pdf.text(`${dados.hospital.abrev ?? '?'} · ${p.duracao}h`, cx + 2.8, by + 2.6);
      pdf.setFont('Nunito', 'normal');
      pdf.setFontSize(6);
      pdf.text(`${horaIni}–${horaFim}`, cx + 2.8, by + 5.4);
...
function desenharDetalhamento(
  ...
  for (const p of ordenados) {
    ...
    const dow = capitalize(DOWS_LONG[d.getDay() === 0 ? 6 : d.getDay() - 1] ?? '');
    const fim = (p.horaInicio + p.duracao) % 24;
    const horaIni = `${String(Math.floor(p.horaInicio)).padStart(2, '0')}:${String(Math.round((p.horaInicio % 1) * 60)).padStart(2, '0')}`;
    const horaFim = `${String(Math.floor(fim)).padStart(2, '0')}:${String(Math.round((fim % 1) * 60)).padStart(2, '0')}`;
  ...
  pdf.setFontSize(13);
  pdf.setTextColor(COR_LAVENDER);
  pdf.text(`${dados.plantoes.length} plantões`, x + largura, 184, { align: 'right' });
}
...
function trataChefe(nome: string): string {
  // Se já vem com Dr/Dra, mantém. Senão prefixa Dr(a).
